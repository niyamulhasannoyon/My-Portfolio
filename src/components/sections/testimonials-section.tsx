  ...
  return (
    <section className="bg-brand-bg py-20">
      <Container>
        ...
        <ul className="mt-12 grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((t, i) => (
            <li key={t.name}>
              <StaggerCard
                index={i}
                as="figure"
                className="flex h-full flex-col rounded-2xl border border-white/[0.06] bg-white/[0.03] p-6 transition-all duration-300 hover:border-emerald-500/20 hover:bg-emerald-500/[0.04]"
              >
